...
- 정산 월을 바꾸면 제목과 본문이 해당 월에 맞게 바뀌며, 이후 직접 편집할 수 있습니다.
- 받는 이메일이 있으면 확인 후 발송하고, 비워 두면 발송하지 않는 Gmail 초안을 저장합니다.
- 발송 또는 초안 저장 결과가 불확실하면 자동 재시도하지 말고 Gmail 보낸편지함이나 임시보관함을 먼저 확인하세요.
- APK를 업데이트할 때는 기존과 동일한 release keystore를 사용해야 합니다.
## 공식 문서
